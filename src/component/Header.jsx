...
function Header() {
    ...
    //account list가 현재 보여질지 보여지지 않을지를 결정할 boolean 변수
    const isVisible = useRecoilValue(viisibilityAccountList);
        ...
        <div id="headerAllContainer">
            <form id="headerContainer">
                ...
                ? <div id="accountBtnContainer">
                    <button id="accountBtn">{window.sessionStorage.id}</button>
            ...
            <ButtonGroup vertical id="accountBtnClickList" style={{visibility: isVisible ? 'visible' : 'hidden'}}>
                <Button variant="outline-secondary">프로필 설정</Button>
                <Button variant="outline-secondary">찜한 게임</Button>
                <Button variant="outline-secondary">로그아웃</Button>
            </ButtonGroup>
        </div>
    )
}
